...
- 기본 파티션 Pruning 기법
- 고급 파티션 Pruning 기법: 서브쿼리 Pruning, 조인 필터 Pruning
  <br/>
...
|   0 | SELECT STATEMENT            |      |     1 |    13 |       |       |
|   1 |   SORT AGGREGATE            |      |     1 |    13 |       |       |
|   2 |     PARTITION RANGE ITERATOR|      |  2310 | 30030 |   KEY |   KEY |
|   3 |       TABLE ACCESS FULL     | T    |  2310 | 30030 |   KEY |   KEY |
----------------------------------------------------------------------------
```
...
id 2번 라인은 Range 파티션에 대한 Pruning 정보를, id 3번 라인은 해시 파티션에 대한 Pruning 정보를 표시하고 있다.
...
|   4 |         PARTITION HASH ALL    |      |  2226 | 28938 |     1 |    16 |
...
파티션 키 컬럼을 명시적으로 가공했을 때와 같은 블록 I/O가 발생한 것을 볼 수 있고, 내부적인 형변환 때문임을 아래 Predicate 정보를 통해 알 수 있다.
```
...
주문은 대용량 거래 테이블이므로 주문일자 기준으로 월별 Range 파티셔닝 돼 있을테고, 일자 테이블을 드라이빙해 NL 조인한다면 분기 >= 'Q20071' 기간에 포함되는 주문 레코드만 읽을 수 있다.
...
```
select distinct BL$OR$IDX$PART$NUM(주문, 0, 1, 0, a.일자)
...
따라서 서브쿼리 Pruning 적용 여부는 옵타마이저가 비용을 고려해 내부적으로 결정한다. 옵티마이저 결정에 영향을 미치는 Hidden 파라미터는 아래 표와 같다.
...
|파라미터명|기본값|설명|
|:---:|:---:|:---|
|_subquery_pruning_cost_factor|20|파티션 테이블이 recursive 서브쿼리 대상 테이블보다 20배 이상 클 때만 이 방식을<br/>사용하겠다는 뜻이다. 바꿔 말하면, recursive 서브쿼리 수행 비용이 파티션 테이블에<br/>있는 모든 데이터를 액세스하는 비용의 5%를 초과하지 않아야 효과적이라는 가정을<br/>세운 것이다. (1/20=0.05)|
...
먼저 블룸 필터 알고리즘에 대해 알아보자. 블룸 필터에 대한 더 자세한 설명은 WIKIPEDIA(http://en.wikipedia.org/wiki/Bloom_filter)에서 얻을 수 있다.
...
설명만으로는 쉽게 이해하기 어려울 것이므로, 예시를 보면서 단계별로 다시 설명해 보자.

- [1] 0부터 9번까지 10개 비트를 할당하고 모두 0으로 설정한다.
- [2] 0부터 9까지 총 10개 값을 리턴할 수 있는 두 개의 함수(h1, h2)를 정의한다. 두 함수는 서로 다르게 구현되었다.
...
- [3] 집합 A의 각 원소(175, 442, 618)마다 차례로 h1, h2 함수를 적용하고, 각 해시 함수에서 리턴된 값(0~9 중 하나)에 해당하는 비트르 모두 1로 설정한다.
  ...
  따라서 3번 과정을 모두 완료하고 나서 비트 값을 확인해 보면, 아래처럼 2, 5, 8 비트가 1로 설정돼 있다.

- [4] 교집합을 찾을 준비가 끝났다. 집합 B(175, 327, 432, 548)의 각 원소마다 차례로 h1, h2 함수를 적용하고, 리턴된 값(0~9 중 하나)에 해당하는 비트를 모두 확인한다.
  하나라도 0으로 설정돼 있으면 그 원소는 집합 A에 없는 값이다.
  모두 1로 설정돼 있으면 그 원소는 집합 A에 포함될 가능성이 있는 값이므로 이때만 집합 A를 찾아가 실제 같은 값을 가진 원소가 있는지 찾아본다.
...
먼저 h1, h2 함수를 아래와 같이 수정하고 다시 생성한다.
```
SQL> create or replace function h1(e number) return number
  2  as
  3  begin
...
이들 출력 값으로 비트를 설정하고 나면, 18, 42, 45, 62, 75 비트가 1로 설정된다.

똑같이 집합 B(175, 327, 432, 548)의 함수 출력 값으로 비트를 확인해 보면, 이번에는 집합 A에 실제 값이 있는 175에 대해서만 확인이 필요해진다. false positive가 한 번도 발생하지 않은 것이다.

이처럼 더 많은 비트를 할당(더 많은 공간 필요)하거나 더 많은 해시 함수를 사용(더 많은 시간을 소비)하면 false positive 발생 가능성은 줄어든다.
...
만약, 조인 필터 Pruning 기능을 비활성화 하고 싶다면, _bloom_pruning_enabled 파라미터를 false로 설정하면 된다.
<br/>
<br/>
## (4) SQL 조건절 작성 시 주의사항
...
인덱스 스캔 효율화 원리에서 like보다 between이 유리한 이유를 설명했는데, 지금 본 것처럼 파티션 Pruning을 위해서라도 like보다는 가급적 between 연산자를 이용해 정확한 검색구간을 지정하는
것이 바람직하다.
...
다시 강조하지만, 파티션 설계와 상관없이 옵티마이저가 효율적인 선태을 할 수 있도록 하려면 between 연산자를 사용하기 바란다.
```
select * from 고객
where  가입일 like '200910%'
