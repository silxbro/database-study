...
본 절에서는 대량의 데이터를 처리할 때 테이블 Random 액세스가 가장 큰 부하 요인으로 작용하는 원인을 자세히 설명하고, 이후 이를 해소하기 위한 튜닝 방안을 소개한다.
...
하지만 보는 시각에 따라서는 '논리적 주소정보'라고 표현하기도 한다. rowid가 물리적 위치정보로 구성되지만 **인덱스에서 테이블 레코드로 직접 연결되는 구조는 아니기** 때문이다.
(IOT 테이블 로우에 대한 내부 식별자를 Logical ROWID라고 하는데 이것을 말하는 것이 아니며, 일반 힙 구조 테이블에서 사용되는 Physical ROWID가 내용적으로 논리적 의미를 담고 있다는 뜻이다.)

### [메인 메모리 DB와의 비교]
메인 메모리 DB(MMDB)는 말 그대로 데이터를 모두 메모리에 로드해 놓고 메모리를 통해서만 I/O를 수행하는 DB라고 할 수 있다.
...
메인 메모리 DB도 벤더에 따라 내부적으로 다른 아키텍처를 사용하겠지만 어떤 제품의 아키텍처를 소개함으로써 방금 던진 질문에 대한 답을 찾고자 한다.
...
오라클에서 하나의 레코드를 찾아가는 데 있어 가장 빠르다고 알려진 rowid에 의한 테이블 액세스가 경우에 따라서는(→ 위 설명은 가장 최악의 상황을 가정한 것임) 이렇게까지 고비용일 수 있다.
<br/><br/>

요약하면, 인덱스 rowid는 테이블 레코드와 물리적으로 연결돼 있지 않기 때문에 인덱스를 통한 테이블 액세스는 생각보다 고비용 구조다.
설령 모든 데이터가 메모리에 캐싱돼 있더라도 테이블 레코드를 찾기 위해 매번 **DBA를 해싱하고 래치 획득 과정을 반복**해야 하기 때문이며, **동시 액세스가 심할 때는 래치와 버퍼 Lock에 대한 경합까지
발생**한다.
...
있다면) 그 레코드들고 가까운 시점에 읽힐 가능성이 높다. 따라서 인덱스를 스캔하면서 읽는 테이블 블록들의 캐시 히트율이 높아지므로 물리적인 디스크 I/O 횟수가 감소하게 되는 것은 당연한 이치다.
...
> 물리적으로 읽어야 할 전체 테이블 블록 개수는 고정(앞선 에에서는 709개)돼 있기 때문이다.
...
똑같은 개수의 레코드를 읽는데 인덱스 CF에 따라 논리적인 블록 I/O 개수가 이처럼 심하게 차이 나는 이유는 무엇일까?
이유는, **인덱스를 통해 액세스되는 하나의 테이블 버퍼 블록을 Pinning**하기 때문이다.

버퍼 Pinning에 대해서 간단히 요약하면, **방금 액세스한 버퍼에 대한 Pin을 즉각 해제하지 않고 데이터베이스 Call 내에서 계속 유지하는 기능**이라고 할 수 있다.
...
이처럼 CF가 좋을 때는(완전히 같은 순서로 정렬돼 있을 필요는 없으며, 같은 테이블 블록에 모여 있기만 하면 됨) 인덱스를 통해 12개의 테이블 레코드를 읽는데 Random 블록 I/O가 단 2회만 발생한다.
<br/><br/>

\[그림 1-19\](p.740)는 모든 화살표가 굵은 실선인 것을 볼 수 있다.
똑같이 12개의 사원 레코드를 인덱스를 통해 읽는데 매번 해시 체인 래치 획득 과정을 거쳐 무겁게 논리적 블록 읽기를 수행함을 표현한 것이다.

이처럼 CF가 나쁘면 최악에는 12개 레코드를 읽기 위해 Random 블록 I/O가 12번 발생한다.
<br/>
<br/>
## (3) 인덱스 손익분기점
...
따라서 CF가 나쁘면 손익분기점은 5% 미만에서 결정되며, 심할 때는(BCHR가 매우 안 좋을 때) 1% 미만으로 떨어진다.
...
즉, 100,000개 레코드 중에서 500개 레코드를 넘는 순간 이미 Full Table Scan에 비해 인덱스 스캔이 더 느려졌다.
...
또한, Full Table Scan 할 때는 CF에 상관없이 양쪽 모두 평균 2.5\~3초 수준에 머문다는 사실을 확인할 수 있다.
<br/><br/>
...
### [손익분기점을 극복하기 위한 기능들]
손익분기점 원리에 따르면 선택도가 높은 인덱스는 효용가치가 낮지만 대용량 테이블을 Full Scan하는 것 역시 비효율이 크다.
...
그리고 인덱스 리프 블록이 곧 데이터 블록이어서 인덱스를 수직 탐색한 다음에 테이블 레코드를 읽기 위한 추가적인 Random 액세스가 불필요하다.
...
읽고자 하는 데이터가 많을 때는 인덱스를 이용하지 않는 편이 낫다고 하지만, 수천만 건에 이르는 테이블을 Full Scan해야 한다면 난감하기 그지없다.
그럴 때, 대량 범위 조건으로 자주 사용되는 컬럼 기준으로 테이블을 파티셔닝한다면 Full Table Scan 하더라도 일부 파티션만 읽고 멈추도록 할 수 있다.

클러스터는 기준 키 값이 같은 레코드를 블록 단위로 모아 놓지만 파티션은 세그먼트 단위로 모아 놓는 점이 다르다.