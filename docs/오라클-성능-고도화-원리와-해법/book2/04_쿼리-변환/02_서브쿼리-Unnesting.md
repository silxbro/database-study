...
특히, 서브쿼리가 메인쿼리에 있는 컬럼을 참조하는 형태를 '상관관계 있는(Correlated) 서브쿼리'라고 부른다.

**3. 스칼라 서브쿼리 (Scalar Subquery)** : 한 레코드당 정확히 하나의 컬럼 값만을 리턴하는 것이 특징이다.
주로 select-list에서 사용되지만 몇 가지 예외사항을 뺀다면 컬럼이 올 수 있는 대부분 위치에서 사용 가능하다.
<br/>
<br/>
...
서브쿼리를 풀어내는 두 가지 쿼리 변환 중 '서브쿼리 Unnesting'은 중첩된 서브쿼리(Nested Subquery)와 관련 있고, '뷰 Merging'은 인라인 뷰와 관련 있다.
(뷰 Merging은 인라인 뷰 뿐만 아니라 '저장된 뷰'에도 작동한다.)
<br/>
<br/>
## (2) 서브쿼리 Unnesting의 의미
'서브쿼리 Unnesting'은 중첩된 서브쿼리(Nested Subquery)를 풀어내는 것을 말하며, 풀어내지 않고 그대로 두는 것은 '서브쿼리 No-Unnesting'이라고 말할 수 있다.

'중첩된 서브쿼리(nested subquery)'는 메인쿼리와 부모와 자식이라는 **종속적이고 계층적인 관계**가 존재한다. 따라서 논리적인 관점에서 그 처리과정은 IN, Exists를 불문하고 필터 방식이어야 한다.
즉, 메인 쿼리에서 읽히는 레코드마다 그 서브쿼리를 반복 수행하면서 조건에 맞지 않는 데이터를 골라내는 것이다.
...
> where 조건절에 사용된 서브쿼리가 [1] 메인쿼리와 상관관계에 있지 않으면서(Non-Correlated, 서브쿼리에서 메인 쿼리를 참조하지 않음) [2] 단일 로우를 리턴(single-row subquery)하는
> 아래와 같은 형태의 서브쿼리를 처리할 때 나타나는 방식이다. (첫 번째는 스칼라 서브쿼리지만 두 번째는 두 개 컬럼을 리턴하므로 스칼라 서브쿼리가 아니다.)
>
> ```
...
- **no-unnest** : 서브쿼리를 그대로 둔 상태에서 필터 방식으로 최적화하도록 유도한다.
  <br/>
...
그리고 이것은 뷰 Merging 과정을 거쳐 최종적으로 아래와 같은 형태가 된다.
...
아래는 서브쿼리에 unnest 힌트를 주고 실행계획을 확인한 결과다. 서브쿼리인데도 일반적인 Nested Loop 조인 방식으로 수행된 것을 볼 수 있다. 위 조인문을 수행할 때와 정확히 같은 실행계획이다.
```
select * from emp
...
여기서 주목할 점은, **Unnesting에 의해 일반 조인문으로 변환된 후에는 emp, dept 어느 쪽이든 드라이빙 집합으로 선택될 수 있다**는 사실이다.
...
|*  3 |     INDEX UNIQUE SCAN| DEPT_PK |     1 |     3 |      1   (0)| 00:00:01 |
---------------------------------------------------------------------------------
...
(이상한 일이지만 10g부터는 아래와 같이 하더라도 조인 순서가 조정된다.)
```
...
이것을 통해, Unnesting 된 서브쿼리가 from절에서 앞쪽에 위치함을 알 수 있다.
```
...
- 1쪽 집합임을 확신할 수 없는 서브쿼리 쪽 테이블이 드라이빙된다면, 먼저 **sort unique 오퍼레이션**을 수행함으로써 1쪽 집합으로 만든 다음에 조인한다.
...
직접 테스트해 보는 과정에서 위와 같은 실행게획이 안 나타난다면(즉, 옵티마이저가 세미 조인 방식을 선택한다면) 아래와 같이 힌트를 사용해 유도할 수 있다.
...
|   2 |     TABLE ACCESS FULL| EMP      |    10 |   350 |      3   (0)|
...
기본적으로 NL 조인과 동일한 프로세스로 진행하지만, Outer(=Driving) 테이블의 한 로우가 Inner 테이블의 한 로우와 **조인에 성공하는 순간 진행을 멈추고 Outer 테이블의 다음 로우를 계속 처리**
하는 방식이다. 아래 pseoudo 코드를 참고한다면 어렵지 않게 이해할 수 있다.
```
...
만약 옵티마이저가 세미 조인 방식을 선택하지 않는다면 아래와 같이 힌트를 사용해 유도할 수 있다. Unnesting한 다음에 메인 커리 쪽 테이블이 드라이빙 집합으로 선택되도록 하는 것이다.
```
select /*+ leading(emp) */ * from emp
...
-------  ---------------------------------------------------------------------
   ...
   1400      TABLE ACCESS FULL T_EMP (cr=12 pr=0 pw=0 time=7049 us)
      1      TABLE ACCESS BY INDEX ROWID DEPT (cr=6 pr=0 pw=0 time=122 us)
      1        INDEX UNIQUE SCAN DEPT_PK (cr=3 pr=0 pw=0 time=55 us) (Object ID 57571)
```
...
NL 세미 조인에서도 이런 캐싱 효과가 가능하다. 하지만 9i까지는 캐싱 효과가 나타나지 않았다. 아래는 9i에서 NL 세미 조인으로 수행했을 때의 결과다.
```
select count(*) from t_emp t
...
그리고 NL조인에서 Inner 쪽 인덱스 루트 블록에 대한 버퍼 Pinning 효과도 여기서도 나타난다.
위 Row Source Operation에서 dept_pk 인덱스에 대한 탐색이 1,400번 일어났지만 cr은 단 2개에 그친 것을 확인할 수 있다. (앞서 본 필터 방식에서 이 효과가 안 나타난 것도 확인할 수 있다.)
...
- exists 필터 : 조인에 성공하는 (서브) 레코드를 만나느 순간 결과집합에 담고 다른 (메인) 레코드로 이동한다.
...
|   2 |     TABLE ACCESS FULL| DEPT         |     4 |    80 |      3   (0)|
|   3 |     INDEX RANGE SCAN | EMP_DEPT_IDX |     9 |    27 |      0   (0)|
---------------------------------------------------------------------------
...
|   4 |     SORT UNIQUE                |              |    14 |    42 |      2  (50)|
|   5 |       INDEX FULL SCAN          | EMP_DEPT_IDX |    14 |    42 |      1   (0)|
-------------------------------------------------------------------------------------
...
NL Anti 조인과 머지 Anti 조인은 기본 처리루틴이 not exists 필터와 같지만, 해시 Anti 조인은 조금 다르다. 해시 Anti 조인으로 수행할 때는, 먼저 dept를 해시 테이블로 빌드한다.
...
10g부터 옵티마이저가 선택할 수 있는 옵션이 한 가지 더 추가되었는데, 서브쿼리로부터 전환된 인라인 뷰를 제거하과 아래와 같이 메인 쿼리에 분석 함수를 사용하는 형태로 변환하는 것이다.
이 기능은 _remove_aggr_subquery 파라미터에 의해 제어되며, 비용기반으로 작동한다.
```
select deptno, dname, empno, ename, sal
from  (
  select d.depnto, d.dname, e.empno, e.ename, e.sal
...
Pushing 서브쿼리는 이처럼 **실행계획 상 가능한 앞 단계에서 서브쿼리 필터링이 처리되도록 강제하는 것**을 말하며, 이를 제어하기 위해 사용하는 옵티마이저 힌트가 push_subq이다.
(인라인 뷰 안에 조건절을 밀어 넣는 '조건절(Predicate) Pushing'과 헷갈리지 말기 바란다.)

**Pushing 서브쿼리는 Unnesting 되지 않은 서브쿼리에만 작동**한다는 사실을 기억할 필요가 있다. 따라서 push_subq 힌트는 항상 no_unnest 힌트와 같이 기술하는 것이 올바른 사용법이다.
만약 옵티마이저가 서브쿼리를 Unnesting 하기로 결정한다면 push_subq 힌트는 무용지물이 되기 때문이다.

push_subq 힌트가 잘 작동하지 않을 때, 대부분 no_unnset 힌트를 같이 사용하지 않은 경우였다. 그리고 9i와 10g 사이에 push_subq 힌트를 기술하는 위치가 바뀐 것에도 기인한다.
```
-- 오라클 9i에서 push_subq 힌트 사용하기
select /*+ PUSH SUBQ leading(e1) use_nl(e2) */ sum(e1.sal), sum(e2.sal)
...
where  e1.no = e2.no
and    e1.empno = e2.empno
and    exists (select /*+ NO_UNNEST PUSH SUBQ */ 'x' from dept
          where deptno = e1.deptno
          and   loc = 'NEW WORK')
```
...
-------  ---------------------------------------------------------------------
  ...
  14000        TABLE ACCESS FULL EMP1 (cr=95 pr=0 pw=0 time=42023 us)
  14000        TABLE ACCESS BY INDEX ROWID EMP2 (cr=28002 pr=0 pw=0 time=404323 us)
...
Parse         1     0.000        0.000       0         0          0          0
Execute       1     0.000        0.000       0         0          0          0
Fetch         2     0.125        0.129       0      6013          0          1
-------- ------ --------- ------------ ------- --------- ---------- ----------
Total         4     0.125        0.129       0      6013          0          1

Rows     Row Source Operation
-------  ---------------------------------------------------------------------
      1  SORT AGGREGATE (cr=6103 pr=0 pw=0 time=128934 us)
   3000    NESTED LOOPS (cr=6103 pr=0 pw=0 time=153252 us)
