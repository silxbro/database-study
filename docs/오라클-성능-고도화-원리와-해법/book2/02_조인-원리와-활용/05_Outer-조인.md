...
DBMS에 실제 Not Null 제약을 생성했건 안 했건 그 컬럼에 Null 값이 들어올 수 없도록 하려는 것이 설계자의 의도이므로 프로그램도 그에 맞게 개발해야 한다.
...
<br/>
## (2) Outer 소트 머지 조인
소트 머지 조인은 소트된 중간 집합을 이용한다는 점만 다를 뿐 처리루틴이 NL 조인과 다르지 않다고 했다.
...
1   0   MERGE JOIN (OUTER) (Cost=9 Card=14 Bytes=2K)
...
오라클은 이런 사실을 감안해 **Outer 테이블을 해시 테이블로 빌드(Build)하는 알고리즘**을 애초에 선택하였다.
Inner 조인하고 나서 해시 테이블을 전체적으로 한 번 더 스캔하는 비효율을 감수하면서까지 말이다. 이유는, 작은 쪽 집합을 해시 테이블로 빌드하는 게 유리하기 때문이다.
<br/><br/>

그런데 \[그림 2-21\](p.277)과 같은 일반적인 엔티티 관계(relationship) 속에서도 주문을 기준으로 고객 또는 상품과 Outer 조인해야 할 필요성이 생긴다.
모델 상으로는 부모 없는 자식 레코드가 있어선 안 되지만 FK를 설정하지 않은 채 DBMS를 운영하는 경우가 많다 보니 실제 그런 레코드들이 많이 생긴다. \[그림 2-21\](p.277) 같은 상태가 되는 것이다.
...
이런 성능 이슈를 해결하려고 오라클은 10g부터 **Inner쪽 집합을 해시 테이블로 빌드할 수 있는 알고리즘을 추가**하게 되었다.
...
- [4] Probe 단계가 끝나면 Right Outer 해시 조인한 것과 동일한 결과집합이 만들어진다.
  이제 해시 테이블을 스캔하면서 체크 표시가 없는 출금 레코드를 결과집합에 삽입(\[그림 2-20\](p.275)함으로써 Full Outer 조인을 완성한다.
  입금액이 null인 레코드가 마지막에 출력된 이유가 바로 이것이다.
