...
begin
  for outer in (select deptno, empon, rpad(ename, 10) ename from sorted_emp)
  loop    -- outer 루프
    for inner in (select dname from sorted_dept where deptno = outer.deptno)
    loop  -- inner 루프
      dbms_output.put_line(outer.empno||' : '||outer.ename||' : '||innter.dname);
    end loop;
  end loop;
end;
/
...
> 따라서 편의상 First 테이블과 Second 테이블을 각각 Outer 테이블, Inner 테이블로 명명하기로 한다. 그리고 문맥에 따라 두 가지 방식을 혼용해 설명할 것이다.
...
그림에서 주목할 점은, emp 테이블이 정렬돼 있기 때문에 조인에 실패하는 레코드를 만나는 순간 멈출 수 있다는 사실이다.
예를 들어, deptno=10인 레코드를 찾기 위해 [1]번 스캔을 진행하다가 20을 만나는 순간 멈춘다.

또 한 가지는, 정렬된 emp에서 스캔 시작점을 찾으려고 매번 탐색하지 않아도 된다는 점이다.
예를 들어, deptno=20인 레코드를 찾는 [2]번 스캔은 [1]번에서 스캔하다가 멈춘 지점을 기억했다가 거기서부터 시작하면 된다. **Outer 테이블인 dept도 같은 순서로 정렬돼 있기 때문에 가능한 일**이다.
<br/><br/>
...
전자의 방식을 이용한다면 굳이 Outer 테이블까지 정렬할 이유는 없기 때문이다.
<br/>
<br/>
## (2) 소트 머지 조인의 특징
...
NL 조인은 조인 컬럼에 대한 인덱스 유무에 따라 크게 영향을 받지만 소트 머지 조인은 영향을 받지 않는다.(인덱스가 미리 만들어져 있으면 소트 머지 조인을 좀 더 빠르게 수행할 수는 있다.)
<br/><br/>
...
Random 액세스 위주의 NL 조인이 대용량 처리에 한계를 보일 때마다 소트 머지 조인이 해결사로서 인기를 누리던 시절이 있었다. 하지만 해시 조인의 등장으로 이제 소트 머지 조인의 위상은 예전만 못하다.
대부분 해시 조인보다 느린 성능을 보이기 때문이다.
...
해시 조인과 마찬가지로, 한쪽 집합(Second 테이블)은 전체범위를 처리하고 다른 한쪽(First 테이블)은 일부만 읽고 멈추도록 할 수 있다. First 테이블 조인 컬럼에 인덱스가 있을 때 그렇다.
...
\[그림 2-5\](p.242)는 아래 쿼리의 수행 과정을 도식화한 것이다. 그림에 표시하지 않았지만 먼저 emp 테이블 정렬 결과를 Sort Area에 담는다.
...
\[그림 2-5\](p.242)을 유심히 살펴보면 소트 머지 조인도 부분적으로 부분범위처리가 가능하다는 것을 알 수 있다.
Second 테이블은 항상 정렬을 수행하므로 전체범위처리가 불가피하지만 First 테이블만큼은 중간에 읽다가 멈출 수 있다는 뜻이다.
...
두 번 Fetch하면서 11건을 읽기 위해 총 19.6초의 수행 소요시간을 보였다. 그리고 t_emp 테이블 소트 과정에서 27,631개의 디스크 읽기와 28,869개의 디스크 쓰기가 발생한 사실도 알 수 있다.
<br/><br/>

이제 큰 t_emp 테이블이 First 테이블이 되도록 소트 머지 조인을 수행해 보자. 부분범위처리 효과를 얻기 위해 t_emp_idx를 이용하도록 index 힌트를 명시했다.

```
select /*+ ordered use_merge(d) index(e t_emp_idx) */ *
from   t_emp e, dept d
where  d.deptno = e.deptno
...
-------  ----------------------------------------------------
     ...
     11    INDEX RANGE SCAN T_EMP_IDX (cr=4 pr=5 pw=0 time=31949 us)
...
아래와 같이 인라인 뷰에 order by절을 명시해 sort group by로 유도하면 First 집합에 대한 Sort Join 오퍼레이션이 사라진다. 9i에서는 order by 절을 추가하지 않더라도 아래와 같이 실행된다.
...
위 조인문은 \[그림 2-6\](p.247)과 같이 처리되므로 dept 테이블 deptno 기준으로 오름차순(asc) 정렬하도록 order by절을 추가하더라도 sort order by 오퍼레이션이 나타나지 않는다.
