...
인덱스로 소트 연산을 생략할 수 없을 때, Top N 쿼리가 어떻게 작동하는지 알아보자.
...
지금 설명한 알고리즘을 지금부터 'Top N 소트' 알고리즘이라고 부르자.
...
이 알고리즘이 작동하면, 소트 연산(=값 비교) 횟수와 Sort Area 사용량을 최소화해 준다. 예를 들어, page 변수에 1을 입력하면 열 개 원소를 담을 때 열(Array) 공간만 있으면 된다.
...
이 방식으로 처리하면, 대상 집합이 아무리 커도 많은 메모리 공간이 필요하지 않다. 전체 레코드를 다 정렬하지 않고도 오름차순(ASC)으로 최솟값을 갖는 열 개 레코드를 정확히 찾아낼 수 있다.
...
참고로, 많은 메모리 공간이 필요 없다는 사실을 증명하기 위해 아래와 같이 Sort Area를 작게 설정하고 테스트하였다.
```
...
실행계획에서 Stopkey가 사라졌다. 'Top N 소트' 알고리즘이 작동하지 않았다는 뜻이다. 그 결과로 Physial Read(pr=698)과 Physical Write(pw=698)가 발생했다.
같은 양(690 블록)의 정렬을 완료했지만 지금은 디스크를 이용해야만 했다.
...
여기서도 physical read와 physical write가 각각 40개씩 발생하긴 했지만 max 함수 쓸 때보다 훨씬 줄었다. 시간이 8초 가량 덜 소요된 것도 확인할 수 있다.