...
SQL 튜닝뿐 아니라 데이터베이스를 안정적으로 관리하기 위해서는 옵티마이저에 대한 정확한 이해가 필수이다.
...
가장 단순한 '=' 조건으로 검색할 때의 선택도만 살펴보면, 컬럼 값 종류 개수(Number of Distinct Value, 이하 'NDV')를 이용해 아래와 같이 구한다.
...
- 예를 들어, 상품분류 컬럼에 '가전', '의류', '식음료', '생활용품' 네 개의 값이 있을 때, 아래 조건절에 대한 선택도는 25%(=1/4)다.
  만약 전체 레코드가 10만 건이면, 카디널리티는 2만 5천이다.
...
|DENSITY|'=' 조건으로 검색할 때의 선택도를 미리 구해 놓은 값.<br/>히스토그램이 없거나, 있더라도 100% 균일한 분포를 갖는다면, 1 / NUM_DISTINCT 값과 일치|
...
- 아파트매물 테이블에서 '시도구분' 컬럼에 대한 히스토그램을 원형 그래프로 표현했다고 가정하자. 히스토그램은 이처럼 컬럼 값별로 데이터 비중 또는 빈도를 미리 계산해 놓은 통계정보다.
  실제 데이터를 읽어서 계산해 둔 값이므로 데이터 분포가 많이 변하지 않는 한 거의 정확하다.

오라클 12c에서 사용하는 히스토그램 유형으로는 아래 표처럼 네 가지가 있다.
|히스토그램 유형|설명|
|:---|:---|
...
#### [4] 시스템 통계
시스템 통계는 애플리케이션 및 하드웨어 성능 특성을 측정한 것인데, 아래 항목들을 포함한다.
...
그러다 보니 실제 오라클이 설치된 운영 시스템 사양이 그보다 좋거나 나쁠 때 옵타마이저가 최적이 아닌 실행계획을 수립할 가능성이 생긴다.

시스템 사양뿐만 아니라 애플리케이션 특성(OLTP, DW) 및 동시 트랜잭션 발생량에 따라서도 이들 성능 특성이 달라진다.
...
## (3) 비용 계산 원리
옵티마이저가 통계정보를 어떤 식으로 활용하는지 이해할 필요가 있으므로 단일 테이블을 액세스할 때의 비용 계산 원리를 간단히 살펴보자.
...
#### [비용(Cost)의 정확한 의미]
> 방금 설명한 비용 계산식은 'I/O 비용 모델' 기준이다. I/O 비용 모델을 사용할 때 실행계획에 나타나는 Cost는 '예상 I/O Call 횟수'를 의미한다.
>
