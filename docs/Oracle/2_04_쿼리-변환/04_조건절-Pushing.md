...
어떤 이유에서건 뷰 Merging에 실패했을 때, 옵티마이저는 포기하지 않고 **2차적으로 조건절(Predicate) Pushing을 시도**한다.
이는 **뷰를 참조하는 쿼리 블록의 조건절을 뷰 쿼리 블록 안으로 Pushing하는 기능**을 일컫는다.

조건절이 가능한 빨리 처리되도록 뷰 안으로 밀어 넣는다면, 뷰 안에서의 처리 일량을 최소화(group by 등 연산 작업 최소화, 인덱스 액세스 조건으로 사용)하게 됨은 물론 리턴되는 결과 건수를 줄임으로써
다음 단계에서 처리해야 할 일량을 줄일 수 있다.
...
- **조인 조건(Join Predicte) Pushdown** : NL 조인 수행 중이 드라이빙 테이블에서 읽은 값을 건건이 Inner 쪽(=right side) 뷰 쿼리 블록 안으로 밀어 넣는 것을 말함
...
### [Non-pushable View]
뷰 안에 rownum을 사용하면 Non-mergeable View가 되는 동시에 Non-pushable View가 된다.
...
분석함수(Analytic Function)를 사용해도 Non-mergeable, Non-pushable View가 되므로 주의가 필요하다.
<br/>
<br/>
## (1) 조건절 Pushdown
...
아래 쿼리는 group by절을 포함한 복합 뷰 사례인데, 9i 이후로는 복합 뷰 Merging이 기본적으로 활성화되므로 뷰 Merging이 발생할 것이다.
이를 막도록 _optimizer_view_merging 파라미터를 false로 바꾸거나 no_merge 힌트를 사용할 때의 실행계획을 확인해 보자.
```
...
이것은 '조인 조건 Pushdown'과 구분되어야 한다. NL 조인을 수행하는 동안 dept 테이블에서 읽은 조인 컬럼(deptno) 값을 건건이 뷰 쿼리 블록에 pushdown 한 것이 아니기 때문이다.

여기서는 인라인 뷰 자체적으로 사전에 deptno = 30 조건절을 적용해 데이터량을 줄이고, group by를 하고 나서 조인에 참여하였다.
deptno = 30 조건이 인라인 뷰에 pushdown 될 수 있었던 이유는, '조건절 이행'이 먼저 일어났기 때문이다.
b.deptno = 30 조건이 조인 조건을 타고 a쪽에 전이됨으로써 아래와 같이 a.deptno = 30 조건절이 내부적으로 생성된 것이다.

```
select /*+ no_merge(a) */
       b.deptno, b.dname, a.avg_sal
from  (select deptno, avg(sal) from emp group by deptno) a
     , dept b
where  a.deptno = b.deptno
and    b.deptno = 30
and    a.deptno = 30
...
이 상태에서 a.deptno = 30 조건절이 인라인 뷰 안쪽으로 Pushing된 것이므로 일반적인 조건절 Pushing으로 이해해야 한다.
...
아래는 조인 조건을 타고 전이된 상수 조건이 뷰 쿼리 블록에 Pushing 된 경우다.
```
...
아래는 Predicate Move Around 기능이 작동하지 않을 때 어떤 비효율이 발생하는지를 보여준다.
opt_param 힌트를 이용해 이 기능을 비활성화시켰고, 이 때문에 9번 단계에서 Index Full Scan이 나타난 것을 확인할 수 있다. (상황에 따라 Full Table Scan이 나타날 수도 있다.)
```
...
## (3) 조인 조건 Pushdown
'조인 조건(Join Predicate) Pushdown'은 말 그대로 조인 조건절을 뷰 쿼리 블록 안으로 밀어 넣는 것으로서, **NL 조인 수행 중에 드라이빙 테이블에서 읽은 조인 컬러 값을 Inner 쪽(=right side)
뷰 쿼리 블록 내에서 참조할 수 있도록 하는 기능**이다.

조인 조건 Pushdown은 조건절 Pushdown의 일종이지만 이를 따로 분류한 이유가 있다.
조인문에서의 조건절 Pushdown은 상수 조건이 조인 조건을 타고 전이된 것을 Pushing하는 기능이었던 반면, 조인 조건 Pushdown은 조인을 수행하는 중에 드라이빙 집합에서 얻은 값을 뷰 쿼리 블록 안에
실시간으로 Pushing하는 기능이다.
...
조인 조건 Pushdown이 일어난 것이며, 실행계획상에 'VIEW PUSHED PREDICATE' 오퍼레이션이 나타나는 것을 통해서도 이를 알 수 있다.
...
group by를 포함하는 뷰에 대한 조인 조건 Pushdown 기능은 11g에 와서야 제공되기 시작했다.

아래는 10g 실행계획이며, 조인 조건 Pushdown이 작동하지 않아 emp 쪽 인덱스를 full scan하는 것을 확인할 수 있다. dept 테이블에서 읽히는 deptno마다 emp 테이블 전체를 group by 하고 있다.
...
아래는 11g에서 확인된 실행계획이다. 'VIEW PUSHED PREDICATE' 오퍼레이션(id=3)이 나타났고, emp_deptno_idx 인덱스를 통해 emp 테이블을 액세스한다.
...
|   2 |     TABLE ACCESS BY INDEX ROWID| DEPT           |     5 |    35 |
|   3 |       INDEX RANGE SCAN         | EMP_DEPTNO_IDX |     5 |       |
|   4 |   TABLE ACCESS FULL            | DEPT           |     4 |    44 |
-------------------------------------------------------------------------
```

집계함수가 두 개 이상일 때가 문제인데, 필요한 컬럼 값들을 모두 결합하고서 바깥쪽 액세스 쿼리에서 substr 함수로 다시 분리하거나, 오브젝트 TYPE을 사용하는 방식을 고려해 볼 수 있다.
...
참고로, 9i에서는 'union-all partition'이라고 표시되는데, 이는 파티션 뷰(Partitioned View)에 대한 Pruning 기능이 작동할 때 보던 것이어서 의미가 상통한다.
(union all을 이용해 뷰를 정의함으로써 수동으로 파티션 기능을 구현하는 것을 '파티션 뷰'라고 한다.)

9i에서 use_nl 힌트를 push_pred와 함께 사용하면 조인 조건 Pushdown 기능이 작동하지 않는 현상이 나타난다.
...
```
select /*+ push_pred(b) */
       a.empno, a.ename, a.sal, a.hiredate, b.deptno, b.dname, a.job
from   emp a
...
뷰 안에서 참조하는 테이블이 단 하나일 때도 no_merge 힌트를 사용해 뷰 Merging을 방지하면 위와 같이 조인 조건 Pushdown이 작동한다.

참고로, 'UNION 집합 연산자를 포함한 뷰에 대한 조인 조건 Pushdown'은 10g부터 비용기반으로 작동하기 시작했지만 Outer 조인 뷰에 대한 기능은 9i에서도 비용기반이었다.
