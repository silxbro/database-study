...
이 외에도 이름이 밝혀지지 않은 많은 쿼리 변환이 작동하고 있다. 그 중 몇 가지 중요한 쿼리 변환만을 소개하고자 한다.
<br/>
<br/>
...
14건을 갖는 emp 테이블을 1,000번 복제했으므로 총 건수는 14,000이고, 모든 레코드의 deptno를 null 값으로 갱신했다.
...
is null 조건을 따로 기술하지 않더라도 읽은 값이 null일 때는 조인 액세스를 하지 않는다는 뜻이며, 이는 매우 중요한 사실이다.
...
Rows     Row Source Operation
-------  ---------------------------------------------------------------------
      1  SORT AGGREGATE (cr=30841 pr=0 pw=0 time=7836 us)
      0    NESTED LOOPS (cr=30841 pr=0 pw=0 time=7815 us)
...
예상실행계획과 함께 출력되는 Predicate 정보를 보면 옵티마이저에 의해 e.deptno is not null 조건이 추가되었음을 알 수 있다.
...
그리고 조인 컬럼에 null 값 비중이 많을 때 임의의 Default 값(0, 'x' 등)으로 채우는 방식으로 설계하면 조인 성능을 떨어뜨릴 수 있다는 사실도 기억하기 바란다.
<br/>
<br/>
## (2) 필터 조건 추가
...
그럴 경우 8i까지는 사용자가 한참을 기다려야 한다. 9i부터는 이를 방지하기 위해 옵티마이저가 임의로 필터 조건식을 추가한다.
아래 실행계획에서 1번 오퍼레이션 단계에 사용된 Filter Predicate 정보를 확인할 수 있다.
```
-------------------------------------------------------------------
...
|*  1 |   FILTER              |           |       |    32 |       |
|*  2 |     TABLE ACCESS FULL | EMP       |     1 |    50 |     2 |
-------------------------------------------------------------------
...
테스트를 위해 아래와 같이 t 테이블을 생성하고, 통계정보를 생성하였다. 10g 환경이므로 기본적으로 CPU Costing 모드가 활성화된 상태다.
```
SQL> create table t
  2  nologging
  3  as
...
아래와 같이 a와 b 컬럼에 대한 조건식을 서로 바꿔 가면서 테스트 해 보았지만 선택도가 낮은 b 컬럼이 항상 먼저 처리되는 것을 볼 수 있다.
```
...
옵티마이저의 판단을 무시하고 아래와 같이 힌트를 썼더니 예상 비용이 433에서 440으로 증가한 사실을 확인할 수 있다. I/O 뿐만 아니라 CPU 연산 시간까지 비용 계산식에 포함하고 있음을 알 수 있다.
```
...
아래와 같이 9i 시스템 통계를 지우거나 10g에서 I/O 비용 모델로 전환한 상태에서 같은 테스트를 수행해 보면, where절에 기술된 순서대로 조건 비교가 일어난다.
```
...
실제 그런지 10g에서 테스트해 보자. 10g에서 같은 컬럼에 대한 OR 또는 IN-List 조건에 OR-Expansion이 작동하도록 하려면 use_concat 힌트에 아래와 같은 인자를 사용해야 한다.
```
SQL> set autotrace traceonly exp
